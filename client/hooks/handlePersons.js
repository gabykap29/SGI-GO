...
export const getFilesByPersonId = async (personId) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/api/persons/${personId}/files`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
        ...
        console.error('Error al obtener archivos de la persona:', error);
        throw error;
    }
};